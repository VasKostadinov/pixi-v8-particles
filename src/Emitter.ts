...
export class Emitter
{
     ...
     * for manual callers.
     */
    private _tickerCallback = (t: Ticker) => this.update(t.elapsedMS * 0.001);
     ...
     */
    /**
     * The class used to construct new particles. Defaults to the classic Sprite-based
     * Particle; pass FastParticle (with a ParticleContainer parent) for the v8 fast path.
     */
    protected _particleClass: ParticleConstructor;
    /** True when the parent container is a v8 ParticleContainer. */
    protected _useParticleContainer: boolean;

    /**
     * @param particleParent The Container (or v8 ParticleContainer) to add particles to.
     * @param config Emitter configuration.
     * @param particleClass Optional explicit Particle constructor. If omitted, the emitter
     *                      auto-selects FastParticle when the parent is a ParticleContainer
     *                      (has `addParticle`), and the classic Particle otherwise.
     */
    constructor(particleParent: Container, config: EmitterConfigV3, particleClass?: ParticleConstructor)
    {
        this._useParticleContainer = typeof (particleParent as any)?.addParticle === 'function';
        this._particleClass = particleClass ?? (this._useParticleContainer ? FastParticle : Particle);
    ...
    */
    ...
    public set parent(value: Container)
    {
        this.cleanup();
        this._parent = value;
        this._useParticleContainer = typeof (value as any)?.addParticle === 'function';
    }

    /** Add a particle to the configured parent, regardless of backend. */
    private _addToParent(p: AnyParticle, atBack: boolean): void
    {
        if (this._useParticleContainer)
        {
            (p as any).parent = this._parent;
    ...
    /** Remove a particle from its parent, regardless of backend. */
    private _removeFromParent(p: AnyParticle): void
    {
        ...
        if (typeof parent.removeParticle === 'function')
        {
            ...
            (p as any).parent = null;
        }
        else
        {
            parent.removeChild(p as any);
        }
    }
    ...
    public set autoUpdate(value: boolean)
    {
        ...
        {
            ticker.remove(this._tickerCallback, this);
        }
        else if (!this._autoUpdate && value)
        {
            ticker.add(this._tickerCallback, this);
        }
        ...
        if (this._emit)
        {
            ...
            while (this._spawnTimer <= 0)
            {
                ...
                for (let len = Math.min(this.particlesPerWave, this.maxParticles - this.particleCount), i = 0; i < len; ++i)
                {
                    ...
                    // initialize particle
                    p.init(lifetime);
                    // add the particle to the display list (dispatches to ParticleContainer if needed)
